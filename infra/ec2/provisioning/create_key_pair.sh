...
if [[ $? -ne 0 ]]; then
    ...
    aws s3api put-object --bucket skysports-my-tf-states --key keys/ubuntu_ec2.pem --body /tmp/ubuntu_ec2.pem
    aws s3api put-object --bucket skysports-my-tf-states --key keys/ubuntu_ec2.pub --body /tmp/ubuntu_ec2.pub
else
    echo "it exists, downloading existing"
    echo "y" | aws s3api get-object --bucket skysports-my-tf-states --key keys/ubuntu_ec2.pem /tmp/ubuntu_ec2.pem
